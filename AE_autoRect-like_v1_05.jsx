/*==============================================================================
    ...
    修正点:
      - 親子付けモード時、シェイプレイヤーの位置を親レイヤー(テキスト)の
        位置に合わせてゼロリセットするように変更。
        これにより、生成位置がコンポ中心にならず、テキストの位置に生成されます。
...
(function (thisObj) {
    ...
    function confirmOverwriteMatte(tgt){
        var hasMatte = (tgt instanceof AVLayer) &&
    ...
    function createAutoRectForTargets(comp, targets, option) {
        ...
        // ------------ 各レイヤーにつき 1 つ ----------
            ...
            for (var i=0;i<targets.length;i++){
                ...
                var insertAbove = option.insertAbove ^ getKeyboardShift();
                if (insertAbove) shape.moveBefore(tgt); else shape.moveAfter(tgt);

                if (option.setTrackMatte && (tgt instanceof AVLayer)) {
    ...
    function getRectProps(layer) {
        ...
        if (auto) {
            var r = auto.property("Contents").property("ADBE Vector Shape - Rect");
            if (r) rects.push(r);
        }
        // 念のため全走査
        for (var i=1; i<=contents.numProperties; i++) {
            ...
            if (p.matchName === "ADBE Vector Group") {
                var rr = p.property("Contents").property("ADBE Vector Shape - Rect");
                if (rr && rects.indexOf(rr) === -1) rects.push(rr);
            }
        }
        return rects;
    }

    // Rect の Size / Position / Roundness を
    // matchName で取得する小ヘルパー
    function getRectSizePosRound(rp) {
        var sz = null, ps = null, rd = null;
        for (var i = 1; i <= rp.numProperties; i++) {
            var p = rp.property(i);
            switch (p.matchName) {
    ...
    function buildUI(thisObj) {
        ...
        btBake.onClick = function(){
            ...
            try {
                ...
                // ---- デバッグ：選択状況と候補レイヤー ----
                (function(){
                    var msg = "[Debug] 選択レイヤー数 : " + (sel ? sel.length : 0) + "\n";
                    if (sel && sel.length > 0) {
                        for (var i = 0; i < sel.length; i++) {
                            var L = sel[i];
                            msg += "  [" + (i+1) + "] " + L.name +
                                   ...
                                   (L instanceof ShapeLayer ? "ShapeLayer" :
                                    L instanceof TextLayer ? "TextLayer" :
                                    L.matchName) +
                                   ")\n";
                        }
                    }
                    msg += "\n[Debug] 矩形候補レイヤー数 : " + cand.length + "\n";
                ...
                // ---- Bake 後に、まだエクスが残っていないかチェック ----
                (function(){
                    var L = cand[0]; // とりあえず1枚目だけ
                    var msg = "[After Bake] 残っているエクスプレッション一覧\n\n";

                    function scanProps(p, prefix){
                        for (var i=1; i<=p.numProperties; i++){
                            var c = p.property(i);
                            var name = prefix + "/" + c.name;

                            if (c.canSetExpression && c.expressionEnabled && c.expression !== ""){
                                msg += "● " + name + " にエクス残っている\n";
                            }
                    ...
                    scanProps(L.property("Contents"), "Contents");
                    alert(msg);
                })();
